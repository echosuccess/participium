...
function checkAuth(req: Request): User {
...
export function requireTechnicalOrExternal(req: Request, res: Response, next: NextFunction) {
  const user = checkAuth(req);
  if (!TECHNICAL_AND_EXTERNAL_ROLES.includes(user.role)) {
...
export function requireCitizenOrTechnical(req: Request, res: Response, next: NextFunction) {
  const user = checkAuth(req);
  if (user.role !== 'CITIZEN' && !TECHNICAL_AND_EXTERNAL_ROLES.includes(user.role)) {
    throw new ForbiddenError("Citizens, technical staff, or external maintainer privileges required");
  }
  return next();
}
