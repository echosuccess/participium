...
const seedDatabase = async () => {
  ...
  const categorySamples: Record<string, { title: string; description: string; preferredRole: Role }> = {
    [ReportCategory.WATER_SUPPLY_DRINKING_WATER]: {
  ...
  for (let i = 0; i < statuses.length; i++) {
    ...
    if (status === ReportStatus.ASSIGNED || status === ReportStatus.IN_PROGRESS) {
    ...
    if (status === ReportStatus.REJECTED) {
      const prUser = createdUsers.find(u => u.role === Role.PUBLIC_RELATIONS) || createdUsers[2];
      await reportMessageRepository.create({
