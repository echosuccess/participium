...
describe('UserDTO', () => {
    describe('toMunicipalityUserDTO', () => {
        ...
        it('handles missing optional fields gracefully', () => {
            const user = { id: 6, email: 'x@y.com', role: UserDTO_1.Roles.MUNICIPAL_BUILDING_MAINTENANCE };
            const dto = (0, UserDTO_1.toMunicipalityUserDTO)(user);
            expect(dto.id).toBe(6);
            expect(dto.email).toBe('x@y.com');
            expect(dto.role).toBe(UserDTO_1.Roles.MUNICIPAL_BUILDING_MAINTENANCE);
        });
        it('handles invalid role in municipality DTO', () => {
            const user = { id: 7, first_name: 'Jane', last_name: 'Doe', email: 'jane@example.com', role: 'INVALID_ROLE' };
            const dto = (0, UserDTO_1.toMunicipalityUserDTO)(user);
