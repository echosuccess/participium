...
import { Roles, MUNICIPALITY_ROLES } from "../../../src/interfaces/UserDTO";
...
>;
const mockUpdateUser = userService.updateUser as jest.MockedFunction<
  typeof userService.updateUser
>;
const mockDeleteUser = userService.deleteUser as jest.MockedFunction<
  typeof userService.deleteUser
>;
const mockFindUsersByRoles =
  userService.findUsersByRoles as jest.MockedFunction<
    typeof userService.findUsersByRoles
  >;
...
describe("municipalityUserService", () => {
  ...
  it("createMunicipalityUser should call createUser and return created user", async () => {
    ...
    expect(mockCreateUser).toHaveBeenCalledWith(
      expect.objectContaining({ email: payload.email, role: payload.role })
    );
  ...
  it("getMunicipalityUserById should return user when role is municipality role", async () => {
    const u = { id: 2, role: Roles.MUNICIPAL_BUILDING_MAINTENANCE } as any;
  ...
  it("getAllMunicipalityUsers should exclude ADMINISTRATOR role", async () => {
    const users = [
      ...
      { id: 2, role: Roles.MUNICIPAL_BUILDING_MAINTENANCE } as any,
    ];
    mockFindUsersByRoles.mockResolvedValue(users);
    ...
    const calledArg = mockFindUsersByRoles.mock.calls[0][0] as any[];
    expect(calledArg).toEqual(MUNICIPALITY_ROLES);
    expect(calledArg).not.toContain(Roles.ADMINISTRATOR);
  });
});
